...
{
    public class CoverageData : BaseData
    {
        public XmlDocument GetXmlForFihr_Coverage(IEnumerable<KeyValuePair<string, string>> parameters, int Count)
        {
            var patientId = parameters.Where(p => p.Key.Equals("patient")).FirstOrDefault().Value;
            var subscriberId = parameters.Where(p => p.Key.Equals("subscriber")).FirstOrDefault().Value;
            ...
            using (var con = new SqlConnection(GetConnectionString()))
            {
                ...
                var cmd = new SqlCommand("sp_GetXmlForFihr_Coverage", con)
                {

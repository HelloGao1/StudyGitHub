...
{
    public class EncounterService : BaseService, IFhirResourceServiceSTU3
    {
        private EncounterData dataAccess;
        public EncounterService()
        {
            dataAccess = new EncounterData();
        }
        ...
        public Bundle Search(IEnumerable<KeyValuePair<string, string>> parameters, int? Count, SummaryType summary)
        {
            ...
            XmlDocument xml = dataAccess.GetXmlForFihr_Encounter(parameters, (int)Count);

            // Map the fields from the stored procedure to Fhir model
            Bundle encounterBundle = EncounterMapping.MapFromCDRToFHirModelBundle(xml, uri);
